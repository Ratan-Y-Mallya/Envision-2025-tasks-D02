...
makeKeymap(keys) function converts the 2D array of the "keys" into a format that the keypad library can understand.
Then we introduce the LCD monitor by using the function lcd(), specifying the address, row and height of the LCD monitor.
The code contains a variable to hold the password input by the user trying to open the vault.
Inside the void setup() function we initialize the LCD monitor and use the function lcd.backlight() to turn on the backlight of the LCD monitor.
The setCursor(column, row) function sets the cursor to the specified position in the plane.
A "Enter password" display command is given to the LCD monitor to be shown during the start of the simulation.
The Serial.begin(9600) initiates a serial communication on an Arduino board. It sets the communication speed with 9600 bauds.
The reserve function reserves the memory for the user to enter the password.
lcd.clear() clears the lcd monitor screen to start with a fresh screen.
